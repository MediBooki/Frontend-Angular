...
export class ReviewComponent implements OnInit {
  ...
  constructor(private _AuthService: AuthService, private _DataService: DataService,private route: ActivatedRoute,private _AppointmentsService: AppointmentsService,private fb: FormBuilder , private router: Router, private toastr: ToastrService) { 
  ...
  submitReview() {
    this._DataService.updatereview.subscribe((res: any) => {
      console.log(res)
      ...
      if(this.reviewStatus === 'false'){
        ...
        this._DataService.createReviewPatient(model).subscribe((res: any) => {
          ...
          this._DataService.isreview.next(true)
        }, error => {
        ...
        console.log(model)
        this._DataService.updateReviewPatient(model).subscribe((res: any) => {
          // console.log(res.data.token);
          console.log(res)
          this.toastr.success(!this.rtlDir?`success`:`تم تسجيل تقييم الدكتور` , `${res.message}`)
          this._DataService.is_login.next(true);
  ...
  getreviewPatient(){
    this.reviewDoctorSubscription = this._DataService.getReviewPatient(this.reviewId).subscribe({
      next: (reviewDoctor) => {
